...
export class AppComponent implements OnInit {
  ...
  ngAfterViewInit(){
    ...
    multiTexturePlane.onReady(function(){
      ...
      const btnNext = document.getElementsByClassName('timeline-nav-button--next');
      ...
      for(let i = 0; i < slideLinks.length; i++) {
        slideLinks[i].addEventListener('click', function(){
          ...
          if(!slider.isAnimating){
            // TODO Need to look at toggling image when returning back to slide
            setTimeout(() =>{
              $('.cd-background-wrapper').hide();
            }, 500);
